...
private struct TrackpadSurfaceArtwork: View {
    ...
    private func drawTrails(context: inout GraphicsContext, size: CGSize) {
        guard let newest = trails.last?.ageIndex else { return }
        for point in trails.suffix(500) {
            let age = max(0, newest - point.ageIndex)
            let opacity = max(0.03, 0.42 - Double(age) / 230)
    ...
    private func drawHeatmap(context: inout GraphicsContext, size: CGSize) {
        ...
        for y in 0..<16 {
            for x in 0..<24 {
                let value = min(max(heatmap[y * 24 + x], 0), 1)
